...
// 실제 경로는 파일 서버만 안다.
// DogfooterImage 가 이미지 파일들을 관리하기는 하지만 그것은 DB 에서 관리되는 상대 경로 및 관계 정보이다.
// Static 한 파일들은 모두 파일 서버(=DogfooterControl)이 관리하도록 한다.
...
func StaticSystemFilePath(separators []string, id string, datetime time.Time, fileName string) (filePath string) {
	...
	filePath = os.Getenv("DOGFOOTER_HOME") + "/system"
...
func StaticProfileAvatarFilePath(account string, datetime time.Time, fileName string) (filePath string) {
	...
	filePath = os.Getenv("DOGFOOTER_HOME") + "/system/account"
	filePath += "/" + account
