...
func (h *Hub) run() {
	for {
		...
		case client := <-h.register:
			...
			//				},
			//			}
			//			WebSocketHub.BroadcastToDogfooter(h.Clients[client.ClientToken].User.Id, rs)
		...
		case client := <-h.unregister:
			if _, ok := h.Clients[client.ClientToken]; ok {
				...
				fmt.Fprintf(os.Stderr, "DEBUG: WebSocket Closed(%v)\n", client.Conn.RemoteAddr())
				c := client
				...
				if c.ClientType == "mate" {
					...
					if WebSocketHub.HasMate(c.User.Id) == false {
						...
						WebSocketHub.BroadcastToDogfooter(c.User.Id, rs)
					}
				}
...
func (h *Hub) BroadcastToDogfooter(userId bson.ObjectId, res WebSocketMessage) {
	for _, v := range h.Clients {
		fmt.Fprintf(os.Stderr, "BroadcastToDogfooter - DEBUG: UserId(%v) - UserId(%v)\n", v.User.Id.Hex(), userId.Hex())
		// 나 자신은 제외
